...
<template>
  <LayoutPage>
    ...
    <div v-for="tx in groupedTransactions" :key="tx.date" class="mt-4">
      <h5 class="mb-3">{{ new Date(`${tx.date}T00:00:00`).toLocaleDateString(undefined, { day: '2-digit', month: 'long' }) }}</h5>
      <div class="list-group bg-white shadow-sm">
        <div v-for="item in tx.items" :key="item.id" class="list-group-item d-flex justify-content-between align-items-center bg-transparent">
          <div>
            <h6 class="mb-1">{{ item.description }}</h6>
            <small class="text-muted">{{ new Date(`${item.date}T00:00:00`).toLocaleDateString() }}</small>
          </div>
